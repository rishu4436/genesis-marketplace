...
class SellerCore:
    ...
    async def _do_work_and_submit(self, job_id: int) -> dict[str, Any]:
        ...
        if spec is not None:
            ...
            task = f"job {job_id}"
        prompt = (
            "You are Gridwright — expert grid-trading strategist for BSC pairs.\n"
            "Produce an EXCELLENT, actionable grid plan (user places orders — no custody).\n\n"
            "Required structure:\n"
            "1) Brief intake — pair, levels, bounds, budget\n"
            "2) Grid parameters — geometric/arithmetic, spacing %, inventory split\n"
            "3) Level map — at least 6 sample prices with BUY/SELL tags\n"
            "4) Risk controls — DD pause %, resume rules\n"
            "5) 24h fill sketch — expected fills and edge before fees\n"
            "6) Execution checklist — numbered\n"
            "7) Metrics — levels, spacing, $ per level, confidence 0-100\n\n"
            "Parse N levels, low/high, pair, % DD from the task. Be numeric.\n"
            "End with: Not financial advice. Agent does not place orders.\n\n"
            f"JOB CONTEXT:\n{task}"
        )
