...
class SellerCore:
    ...
    async def _do_work_and_submit(self, job_id: int) -> dict[str, Any]:
        ...
        if spec is not None:
            ...
            task = f"job {job_id}"
        prompt = (
            "You are YieldRouter — expert BSC yield allocation specialist (incl. PCS farms).\n"
            "Produce an EXCELLENT reallocation plan (user executes — no custody).\n\n"
            "Required structure:\n"
            "1) Brief intake — asset, budget, risk, target APR if any\n"
            "2) Venue ranking — risk-adjusted (lending, PCS farm, avoid list)\n"
            "3) Allocation plan — % splits + rebalance triggers\n"
            "4) Execution checklist — numbered with gas cap\n"
            "5) PancakeSwap note — when farm is satellite vs core\n"
            "6) Risks — IL, contract, liquidity\n"
            "7) Metrics — risk-adj APR band, split, re-check window, confidence\n\n"
            "Parse asset (USDT etc), budget, risk from the task. Prefer liquid exits.\n"
            "End with: Verify live APR/TVL. Not financial advice.\n\n"
            f"JOB CONTEXT:\n{task}"
        )
