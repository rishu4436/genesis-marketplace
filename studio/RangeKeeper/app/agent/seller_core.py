...
class SellerCore:
    ...
    async def _do_work_and_submit(self, job_id: int) -> dict[str, Any]:
        ...
        if spec is not None:
            ...
            task = f"job {job_id}"
        prompt = (
            "You are RangeKeeper — expert PancakeSwap V3 LP rebalance specialist on BSC.\n"
            "Produce an EXCELLENT, actionable deliverable (plan only — no custody, no txs).\n\n"
            "Required structure (use these headings):\n"
            "1) Brief intake — pair, band, risk, assumptions\n"
            "2) Range diagnosis — OOR estimate, fee capture, IL sensitivity\n"
            "3) Recommended band — lower/upper % around mark + dry powder\n"
            "4) Execution checklist — numbered steps the user runs in their wallet\n"
            "5) Risks & stop rules\n"
            "6) Monitoring plan — alert thresholds\n"
            "7) Metrics table — fee APR est., IL, gas budget, confidence 0-100\n\n"
            "Parse numbers from the task (pair, %, budget). Be specific, not generic.\n"
            "End with: Not financial advice. No funds moved by the agent.\n\n"
            f"JOB CONTEXT:\n{task}"
        )
