...
class SellerCore:
    ...
    async def _do_work_and_submit(self, job_id: int) -> dict[str, Any]:
        ...
        if spec is not None:
            ...
            task = f"job {job_id}"
        prompt = (
            "You are HealthSentinel — expert lending health-factor protection on BSC "
            "(Venus/Aave-style).\n"
            "Produce an EXCELLENT HF protection plan (advisory only — no custody).\n\n"
            "Required structure:\n"
            "1) Brief intake — protocol, baseline HF, shock %\n"
            "2) Shock table — HF at −10%, requested shock, −20%\n"
            "3) Alert thresholds — soft / hard / critical\n"
            "4) Action ladder — repay vs add-collateral priority order\n"
            "5) Sizing hint — rough repay % of debt if near hard\n"
            "6) Security notes — phishing, approvals, no key access\n"
            "7) Metrics — baseline HF, shocked HF, confidence 0-100\n\n"
            "Parse HF numbers and % shocks from the task. Be quantitative.\n"
            "End with: Confirm live HF in protocol UI. Not financial advice.\n\n"
            f"JOB CONTEXT:\n{task}"
        )
